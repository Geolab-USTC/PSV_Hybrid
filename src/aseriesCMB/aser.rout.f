...
c 222   format(5x,' to= ',e18.8,5x,' po=',e18.8)
c      write(*,222) to,po
...
c /*                   the previous one                          */
      ...
      common/soray/nsorce,ntype,ndiret,tqc,tqs,tqd,jpt,nconjt,dconjt,nb
      common/travel/rlp(icc),rls(icc),ll,fdp,map,nplnw
      complex*16 e(icc),f(icc)
      complex pc,dpt,tq,cr
       complex p,bl,t,q,psq
c       real*8 c,s,d,th,r
       common/testin/iflag1
c       real*8 rcsq,rssq
...
c               /* crossing the point */
           ...
           if(i.gt.max_num)then
c              write(*,*)"Fail to locate the ti=",ti, " for det0=", det0
               goto 4
           endif

c           write(*,*)"i=",i, " x1m=", x1m, " x4m=", x4m, " ti=", ti
      go to 45
...
c     real*8 psq,rr,e,rcsq,rssq,c,s,d,th,x,f
 ...
 5     rr=rr+rlp(j)*th(j)*e +rls(j)*th(j)*f
     ...
     +,mkp
      ...
      complex pp0(imgc), ddpt0(imgc), tt0(imgc)
...
c     write(*,1001) j,psq,rssq(j),rcsq(j),e(j),f(j)
 ...
 11     totem=totem+e(j)*th(j)*rlp(j)+f(j)*th(j)*rls(j)
        to=p*x+totem
c        write(*,17) k, po, to
c  17  format(4x, 'lmax= ', i6, 4x,'po= ',e18.6,10x,'to= ',e18.6)
      return
      end
...
7     z(i)=c(j1)*fscl
      ...
      subroutine step(nfa,nfad,ndeck,nplot,nprt,dp,njump)
      include 'psv.p'
      common/orst/c(icc),ss(icc),d(icc),th(icc),x
       common/plotc/con
       common/stor/p(itt),td(itt)
        common/thy/t(itt),pp(itt),rp(itt),lz
      common/srf/f(itt)
      dimension dd(10)
      common/comcpl/itest,xlngth,ylngth,xoff,yoff
      common/pltc1/spin,iy
      common/syth/xd11,yd11,xd22,yd22,ym1,ym2,phi,eta
      l=0
      call fa(dp,nfa)
      nff=nfa/2
       do 20 n=1,nff,nfad
      l=l+1
      p(l)=convs(pp,f,dp,nfa-1,n-1)
      p(l)=p(l)*con
       td(l)=2.*dp*(n-1)
 20     continue
c     write(*,500) con
       ...
       do 30 j=2,l
 30     pp(j)=p(j)
c      write(*,300)(pp(j),j=1,64)
      nn=l
        if(njump.gt. 0) go to 2
        ds=2.*dp*nfad
c      s(t)=t**eta*(exp(-phi*t))
c    phi=positive
        dz=2.*ds
        t1=dz**eta*exp(-phi*dz)
        t2=ds**(eta-1)*exp(-phi*ds)*(eta-phi*ds)
        t3=dz**(eta-1)*exp(-phi*dz)*(eta-phi*dz)
       rp(1)=3.*t1/ds-4.*t2-t3
        write(*,400) ds,dz,t1,t2,t3
        do 92 j=2,l
         f(j)=td(j)**eta*exp(-phi*td(j))
 92     rp(j)=td(j)**(eta-1.)*exp(-phi*td(j))*(eta-phi*td(j))
       j2=l
c       write(*,300) (f(j),td(j),j=1,20)
c       write(*,300) (rp(j),j=1,20)
       nff=l/2
         l=0
        do 93 n=1,nff
        l=l+1
        p(l)=convs(pp,rp,ds,j2,n-1)
        td(l)=2.*ds*(n-1)
 93     continue
      do 94 j=1,l
 94     pp(j)=p(j)
c      write(*,300) (td(j),p(j),j=1,l)
       nn=l
 2      if(nplot.lt. 1) go to 3
       itest=1
       dd(1)=0
        dd(3)=2.
      lab=0
      ...
      ymax=ym1
      ymin=-ym1
      if(ym1.gt.0.) go to 46
      tx=0.
        nx=nn
         do 45 j=1,nx
      tx=amax1(abs(pp(j)),tx)
 45     continue
      ymax=tx
      ymin=-ymax
 46     continue
      xlngth=xd11
        ylngth=yd11
 24   xlg1=xlngth/2.+xoff
      xlgnm=xoff+xlngth+.1
      ypt=yoff+3.*yd11/4.
      yptnm=ypt+.15
      if(tx.ne.0.0) go to 12
      ipow=0
      tx1=tx
      go to 13
 12   continue
c      call enum(tx,tx1,ipow)
 13   pow=ipow
c     call number (xlgnm,ypt,.10,tx1,0.,3)
c     call symbol (999.,999.,.10,' e',0.,2)
c     call number (999.,999.,.10,pow,0.,-1)
c      if(iy.eq.1) call symbol(999.,999.,.10,'=max. ampl. ',0.,12)
c      if(iy.eq.1) call number(xlgnm,yptnm,.10,x,0.,2)
cc     if(iy.eq.1) call symbol(999.,999.,.10,'=distance',0.,9)
c     if (lab.eq.1.and.spin.gt.0.0) go to 888
c     go to 889
c888   call number(7.0,0.5,.08,spin,0.,3)
c     call symbol(999.,999.,.08,'sec/inch',0.,8)
c889   call cplot(pp,nn,nn,dd,ymax,ymin,lab)
 889   continue
 3       continue
       return
 100    format(1x,5e15.6)
 200    format(1x,3i10)
 300    format(1x,8e14.4)
  400   format (1x,'ds=',e11.4,'dz=',e11.4,'t1=',e11.4,'t2= ',e11.4,
     *'t3= ',e11.4)
  500 format (1x,'in step,convolved fn. is multiplied by con =',e10.3)
      end
      subroutine fa(del,j2)
      include 'psv.p'
      common/srf/f(itt)
      do 5 j=2,j2
      f(j)     = 1./(((j-1)*del)**.5)
 5      continue
      f(1)     = (11.-4.*2.**.5)/((2.*del)**.5)
      return
         end
      function convs(fa,fp,del,nf,n)
      dimension fp(*),fa(*)
c     computes convolution of fp and fa t=del*2n
c       nf must be odd
      nn=n
      dn=del
      if(nn.lt.1) go to 2
